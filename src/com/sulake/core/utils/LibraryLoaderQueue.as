package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1609:int = 4;
      ...
      {
         return this.var_414.length + this.var_281.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_281.length < const_1609 && this.var_414.length > 0)
            {
               _loc1_ = this.var_414.shift();
               this.var_281.push(_loc1_);
               _loc1_.resume();
            }
         }
      }
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_414.indexOf(loader);
            if(index > -1)
            {
               this.var_414.splice(index,1);
            }
            index = this.var_281.indexOf(loader);
            if(index > -1)
            {
               this.var_281.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_414)
            {
               if(param2)
               {
