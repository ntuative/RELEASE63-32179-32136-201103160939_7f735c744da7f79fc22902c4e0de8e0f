package com.sulake.habbo.advertisement
{
   ...
   public class AdManager extends Component implements IAdManager
   {
      ...
      private function onConfigurationReady(param1:IID = null, param2:IUnknown = null) : void
      {
         ...
         this.var_2503 = this._config.getKey("interstitial.enabled","false") == "true";
         this.var_2504 = int(this._config.getKey("interstitial.interval","30000"));
         this.var_2506 = int(this._config.getKey("interstitial.show.time","5000"));
         this.var_1774 = int(this._config.getKey("interstitial.max.displays.flash","-1"));
      ...
      public function showInterstitial() : Boolean
      {
         ...
         if(this.var_220 != null && this.var_220.running)
         {
            this.var_220.reset();
            return false;
         }
         ...
         if(this.var_1774 > 0 && this.var_1773 >= this.var_1774)
         {
            return false;
         }
         var _loc1_:int = getTimer() - this.var_1156;
         if(_loc1_ > this.var_2504)
         {
            ++this.var_1773;
      ...
      private function onInterstitial(param1:InterstitialMessageEvent) : void
      {
         ...
         this.var_1417 = _loc2_.imageUrl;
         this.var_2505 = _loc2_.clickUrl;
         this.var_1156 = getTimer();
         if(this.var_1417 != "")
         {
            _loc3_ = this.var_1417;
            ...
            {
               this.var_220 = new Timer(this.var_2506,1);
               this.var_220.addEventListener(TimerEvent.TIMER_COMPLETE,this.interstitialTimerComplete);
            }
            if(this.var_220.running)
            {
               this.var_220.reset();
            }
            this.var_220.start();
            _loc4_ = new URLRequest(this.var_1417);
      ...
      private function startInterstitial(param1:Bitmap) : void
      {
         ...
         {
            events.dispatchEvent(new AdEvent(AdEvent.const_574,0,0,param1.bitmapData,this.var_2505));
            this.var_1156 = getTimer();
         }
         else
         {
      ...
      private function stopInterstitial() : void
      {
         ...
         if(events != null)
         {
            events.dispatchEvent(new AdEvent(AdEvent.const_441));
         }
      }
      ...
      private function onRoomAd(param1:RoomAdMessageEvent) : void
      {
         ...
         this.var_1418 = _loc2_.imageUrl;
         this.var_1772 = _loc2_.clickUrl;
         if(this.var_1418 != "" && this.var_1772 != "")
         {
            _loc3_ = this.var_1418;
            if(assets.hasAsset(_loc3_))
            {
               assets.removeAsset(assets.getAssetByName(_loc3_));
            }
            _loc4_ = new URLRequest(this.var_1418);
            _loc5_ = assets.loadAssetFromFile(_loc3_,_loc4_,"image/jpeg");
      ...
      private function roomAdImageReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(this.isValidAdImage(_loc3_))
         {
            ...
            events.dispatchEvent(new AdEvent(AdEvent.const_531,0,0,_loc4_,this.var_1772,this.var_1158,this.var_1157));
         }
      }
