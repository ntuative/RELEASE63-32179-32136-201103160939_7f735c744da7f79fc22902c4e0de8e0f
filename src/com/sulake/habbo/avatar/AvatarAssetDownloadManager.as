package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_959:int = 3;
      
      private var var_427:AssetLoaderStruct;
      ...
      private const const_2020:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_427 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_958 = new Timer(this.const_2020,1);
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_427)
         {
            this.var_427.addEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_427.addEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_427)
         {
            this.var_427.removeEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_427.removeEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_959);
            if(this.var_1461.indexOf("?") > 0)
            {
               _loc2_ = this.var_1461 + "&retry=" + this.var_959;
            }
            else
            {
               _loc2_ = this.var_1461 + "?retry=" + this.var_959;
            ...
            this.var_427 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2596,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1185[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1185[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1838)
         {
            this.var_1186.push([param1,param2]);
            return;
         }
      ...
      {
         var _loc1_:* = null;
         while(this.var_768.length > 0 && this.var_769.length < this.const_1609)
         {
            _loc1_ = this.var_768[0];
            _loc1_.startDownloading();
            this.var_769.push(this.var_768.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_768.indexOf(param1) == -1 && this.var_769.indexOf(param1) == -1)
         {
